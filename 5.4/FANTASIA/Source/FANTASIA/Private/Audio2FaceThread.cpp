...
void FMyThread::SendToAudio2FaceGrpc()
{
    ...
    // First must send a start_marker
    ...
    for (int32 i = 0; i < AudioData.Num(); i += chunk_size) {
        ...
        if (!writer->Write(*requestAudio)) {
            GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red, TEXT("Error while streaming audio's chunks!"));
            break;
        }
       // Sleep(static_cast<DWORD>(round(sleep_between_chunks * 1000))); // converts in milliseconds
